...
try:
    client.collections["Litterae Annuae Societatis Jesu"].delete()
except ObjectNotFound:
    pass

current_schema = {
    "name": "Litterae Annuae Societatis Jesu",
    "fields": [
        {"name": "id", "type": "string"},
...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        pages += 1
        cfts_record = {
            "project": "Litterae Annuae Societatis Jesu",
        }
        record = {}
        record["id"] = os.path.split(x)[-1].replace(".xml", f".html?tab={str(pages)}")
        cfts_record["id"] = record["id"]
        cfts_record["resolver"] = f"https://github.com/jesuiten-oenb/jesuiten-static/{record['id']}"
...
make_index = client.collections[
    "Litterae Annuae Societatis Jesu"
].documents.import_(records)
print(make_index)
print("done with indexing Litterae Annuae Societatis Jesu")

make_index = CFTS_COLLECTION.documents.import_(cfts_records, {"action": "upsert"})
print(make_index)
print("done with cfts-index Litterae Annuae Societatis Jesu")
